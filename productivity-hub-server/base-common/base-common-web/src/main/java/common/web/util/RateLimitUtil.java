...
public class RateLimitUtil {
    ...
    public static boolean tryAcquireForUser(String userId, double qps) {
        ...
        // 如果QPS配置改变，更新限流器的速率
        if (Math.abs(limiter.getRate() - qps) > 0.001) {
            log.debug("[RateLimitUtil] 更新用户限流器QPS：userId={}，旧QPS={}，新QPS={}", userId, limiter.getRate(), qps);
            limiter.setRate(qps);
        }
    ...
    public static boolean tryAcquireForApi(String apiPath, double qps) {
        ...
        // 如果QPS配置改变，更新限流器的速率
        if (Math.abs(limiter.getRate() - qps) > 0.001) {
            log.debug("[RateLimitUtil] 更新接口限流器QPS：apiPath={}，旧QPS={}，新QPS={}", apiPath, limiter.getRate(), qps);
            limiter.setRate(qps);
        }
        return limiter.tryAcquire();
    }
    ...
    public static boolean tryAcquireForIp(String ip, double qps) {
        ...
        // 如果QPS配置改变，更新限流器的速率
        if (Math.abs(limiter.getRate() - qps) > 0.001) {
            log.debug("[RateLimitUtil] 更新IP限流器QPS：ip={}，旧QPS={}，新QPS={}", ip, limiter.getRate(), qps);
            limiter.setRate(qps);
        }
        return limiter.tryAcquire();
    }
