...
public class WaterReminderTask {
    ...
    /**
     * 每小时整点执行（7:00-12:00, 14:00-21:00）
     * 关联需求：REQ-HEALTH-003
     */
    @Scheduled(cron = "0 0 7-12,14-21 * * ?", zone = "Asia/Shanghai")
    ...
    private void sendReminderToUser(String userId) {
        // 从全局配置参数中获取钉钉Webhook地址
        String webhookUrl = configService.getTemplateConfigValue("dingtalk", "dingtalk.webhook");
        
        if (webhookUrl == null || webhookUrl.trim().isEmpty()) {
            log.debug("全局配置中未配置钉钉Webhook地址，跳过提醒");
            return;
        }
