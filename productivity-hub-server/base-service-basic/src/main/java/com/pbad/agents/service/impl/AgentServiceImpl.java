...
public class AgentServiceImpl implements AgentService {
    ...
    public AgentInvokeResponseVO invokeAgent(AgentInvokeDTO invokeDTO, String userId) {
        // 参数校验
        ...
        if ("sync".equals(mode)) {
            ...
            try {
                // 调用第三方API接口
                Map<String, String> configs = new HashMap<>(); // 可以从配置服务获取
                String output = agentApi.invokeAgent(agentId, agent.getName(), invokeDTO.getInput(), configs);
            ...
            // 异步执行
            executeAgentAsync(taskId, agent, invokeDTO.getInput(), userId);
    ...
    @Override
    @Transactional(readOnly = true)
    public PageResult<AgentLogVO> getAgentLogs(int pageNum, int pageSize, String userId) {
        int safePageNum = Math.max(pageNum, 1);
        int safePageSize = Math.min(Math.max(pageSize, 1), 100);
        int offset = (safePageNum - 1) * safePageSize;

        List<AgentLogPO> poList = agentLogMapper.selectPage(userId, offset, safePageSize);
        long total = agentLogMapper.countAll(userId);
     ...
     */
    @Async
    public void executeAgentAsync(String taskId, AgentPO agent, Map<String, Object> input, String userId) {
        try {
            // 调用第三方API接口
            Map<String, String> configs = new HashMap<>(); // 可以从配置服务获取
            String output = agentApi.invokeAgent(agent.getId(), agent.getName(), input, configs);
            int duration = agent.getLatencyMs() != null ? agent.getLatencyMs() : 1000;
