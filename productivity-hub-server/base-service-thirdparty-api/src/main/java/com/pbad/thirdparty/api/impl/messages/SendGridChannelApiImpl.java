 ...
 *
 * <p>说明：
 * - API Key 与发件人邮箱从配置参数传入（configs中的apiKey和fromEmail）。
 * - data 约定字段：recipients(逗号分隔多个邮箱)、subject、content。
 */
@Slf4j
@Service("sendgridChannelApi")
public class SendGridChannelApiImpl implements MessageChannelApi {
    ...
    public String sendMessage(Map<String, Object> data, Map<String, String> configs) {
        ...
        if (apiKey == null || apiKey.isEmpty() || fromEmail == null || fromEmail.isEmpty()) {
            throw new BusinessException("400", "SendGrid 配置不完整，请提供 apiKey 和 fromEmail");
        }
