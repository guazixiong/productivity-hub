 ...
 *
 * <p>说明：
 * - Webhook 地址与加签秘钥从配置参数模块 sys_config 读取（模块：dingtalk，key：dingtalk.webhook / dingtalk.sign）。
...
public class DingTalkChannelServiceImpl implements MessageChannelService {
    ...
    public String sendMessage(Map<String, Object> data) {
        ...
        // 从配置中心读取 webhook 与 sign
        String webhook = configService.getConfigValue("dingtalk", "dingtalk.webhook");
        if (webhook == null || webhook.isEmpty()) {
            throw new BusinessException("400", "DingTalk webhook 未配置，请在配置中心填写 dingtalk.webhook");
        }

        String signSecret;
        try {
            signSecret = configService.getConfigValue("dingtalk", "dingtalk.sign");
