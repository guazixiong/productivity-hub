...
06
    ...
    /**
     * 查询最近一条饮水记录（按饮水日期+时间倒序）.
     *
     * @return 最近一条饮水记录
     */
    @GetMapping("/latest")
    public ApiResponse<WaterIntakeVO> getLatest() {
        try {
            String userId = getCurrentUserId();
            WaterIntakeVO vo = waterIntakeService.getLatest(userId);
            return ApiResponse.ok(vo);
        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.error("查询最近一条饮水记录失败", e);
            return ApiResponse.fail("查询最近一条饮水记录失败：" + e.getMessage());
        }
    }
