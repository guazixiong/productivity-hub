...
public class HealthDailyReportTask {
     ...
     */
    public void sendDailyHealthReport(String targetUserId) {
        if (!isTaskEnabled()) {
            log.info("用户每日健康统计邮件推送任务已被关闭，跳过执行");
            return;
        }

        log.info("开始执行用户每日健康统计邮件推送任务");
        ...
        try {
            ...
            for (UserPO user : enabledUsers) {
                ...
                try {
                    sendReportToUser(healthUser, yesterday, yesterdayStart, yesterdayEnd);
                    successCount++;
                } catch (Exception e) {
                    failCount++;
                    log.error("向用户 {} 发送健康统计邮件失败: {}", userId, e.getMessage(), e);
                }
            }

            log.info("用户每日健康统计邮件推送任务执行完成，成功：{}，失败：{}", successCount, failCount);
        } catch (Exception e) {
     ...
     * 向指定用户发送健康统计邮件
     */
    private void sendReportToUser(HealthUserBodyInfoPO user, LocalDate reportDate, Date startDate, Date endDate) {
        ...
        int targetMl = DEFAULT_DAILY_TARGET_ML;
        ...
        if (waterTarget != null && waterTarget.getDailyTargetMl() != null) {
            targetMl = waterTarget.getDailyTargetMl();
        }
        log.debug("用户 {} 饮水目标: {} ml", userId, targetMl);
        ...
        LocalDate trendEndDate = reportDate;
        ...
        // 构建邮件内容
        String subject = "健康统计日报 - " + reportDate.toString();
        String html = buildReportHtml(user, reportDate, exerciseStats, waterStats, weightStats,
                consumedMl, targetMl, waterAchieved, waterProgress, latestWeight,
        ...
        log.debug("已向用户 {} 发送健康统计邮件", userId);
    }

    /**
     * 构建邮件HTML内容
     */
    private String buildReportHtml(HealthUserBodyInfoPO user, LocalDate reportDate,
                                    Map<String, Object> exerciseStats, Map<String, Object> waterStats,
                                    Map<String, Object> weightStats, int consumedMl, int targetMl,
        ...
        sb.append("<h1>📊 健康统计日报</h1>\n");
        sb.append("<p>").append(reportDate.toString()).append(" 健康数据汇总</p>\n");
        sb.append("</div>\n");
        ...
        sb.append("</div>\n");
        sb.append("<div class=\"stat-item\">\n");
        sb.append("<div class=\"stat-label\">目标饮水量</div>\n");
        ...
        if (waterTrendData != null && !waterTrendData.isEmpty()) {
            sb.append(generateWaterTrendChart(waterTrendData, targetMl));
        } else {
        ...
        sb.append("<p>由 Productivity Hub 自动生成 | ").append(reportDate.toString()).append("</p>\n");
        sb.append("</div>\n");
     ...
     * 生成饮水趋势图（SVG格式）
     */
    private String generateWaterTrendChart(List<Map<String, Object>> trendData, int targetMl) {
        if (trendData == null || trendData.isEmpty()) {
            return "<p style=\"text-align: center; color: #999;\">暂无数据</p>";
        }
        ...
        // 绘制目标线
        int targetY = padding + chartHeight - (targetMl * chartHeight / maxVolume);
        svg.append("<line x1=\"").append(padding).append("\" y1=\"").append(targetY)
            .append("\" x2=\"").append(width - padding).append("\" y2=\"").append(targetY)
            .append("\" stroke=\"#67C23A\" stroke-width=\"2\" stroke-dasharray=\"5,5\"/>\n");
        svg.append("<text x=\"").append(width - padding + 10).append("\" y=\"").append(targetY + 5)
            .append("\" font-size=\"10\" fill=\"#67C23A\">目标: ").append(targetMl).append("ml</text>\n");
        ...
        if (dates.size() > 0) {
            ...
            for (int i = 0; i < dates.size(); i++) {
                ...
                String color = volumes.get(i) >= targetMl ? "#67C23A" : "#409EFF";
