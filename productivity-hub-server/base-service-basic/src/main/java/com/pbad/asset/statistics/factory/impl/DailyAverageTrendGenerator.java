...
public class DailyAverageTrendGenerator implements ChartDataGenerator<StatisticsTrendVO> {
    ...
    public ChartType getType() {
        return ChartType.DAILY_AVERAGE_TREND;
    }
    ...
    public StatisticsTrendVO generate(String userId, StatisticsQueryDTO query) {
        ...
        // 按日期分组，计算每天的日均价格总和
        Map<String, BigDecimal> dailyAverageMap = new TreeMap<>();
        
        // 生成日期列表
        List<String> dates = generateDateList(startDate, endDate);
        for (String date : dates) {
            dailyAverageMap.put(date, BigDecimal.ZERO);
        }
        ...
        for (AssetPO asset : filteredAssets) {
            ...
            BigDecimal additionalFeesTotal = fees.stream()
                    .filter(fee -> fee.getAmount() != null)
                    .map(AssetAdditionalFeePO::getAmount)
            ...
            while (!currentDate.isAfter(effectiveEndDate)) {
                ...
                dailyAverageMap.put(dateStr, dailyAverageMap.get(dateStr).add(dailyAverage));
        ...
        vo.setDates(new ArrayList<>(dailyAverageMap.keySet()));
        vo.setValues(new ArrayList<>(dailyAverageMap.values()));
